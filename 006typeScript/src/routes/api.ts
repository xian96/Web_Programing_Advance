...
export class Api {
	public routes(app): void {
		...
		app.route("/:id").get(async (req: Request, res: Response) => {
			try {
		...
		app.route('/:id').put(async (req: Request, res: Response) => {
			try {
		...
		app.route('/:id').patch(async (req: Request, res: Response) => {
			try {
				// Updates the task with the supplied ID and returns the new task object;
		...
		app.route('/:id/comments').post(async (req: Request, res: Response) => {
			try {
				const id = req.params.id;
				if (!id || typeof id !== 'string')
					throw "You must provide a id for your post update";
		...
		app.route('/:taskId/:commentId').delete(async (req: Request, res: Response) => {
			try {
